...
# Keep the proven v2.33.31 reconstruction/build chain, then splice the v2.33.32 identity-policy
# fix immediately before Gradle and replace only the superseded post-build/runtime assertions.
python3 - <<'PY'
from pathlib import Path
src = Path('scripts/build_v23331_driver.sh').read_text(encoding='utf-8')

old = '''python3 scripts/patch_v23331_live_speaker.py upstream .\npython3 scripts/audit_v23331_live_speaker.py upstream\n\n(\n  cd upstream\n'''
new = '''python3 scripts/patch_v23331_live_speaker.py upstream .\npython3 scripts/audit_v23331_live_speaker.py upstream\npython3 scripts/patch_v23332_live_identity_collapse_fix.py upstream .\npython3 scripts/audit_v23332_live_identity_collapse_fix.py upstream\n\n(\n  cd upstream\n'''
if src.count(old) != 1:
    raise SystemExit('v23332 driver: pre-build v31 anchor mismatch')
