...
def forbid(text: str, needle: str, label: str) -> None:
    if needle in text: raise RuntimeError(f"forbidden {label}: {needle}")
    print("ok:", label)


def main() -> None:
    if len(sys.argv) != 2: raise SystemExit("usage: audit_v2180_adaptive_recovery.py <morphe-root>")
    root = Path(sys.argv[1]).resolve()
    pkg = root / "extensions/youtube/src/main/java/app/morphe/extension/youtube/patches/voiceovertranslation"
    translator = (pkg / "TranscriptTranslator.java").read_text(encoding="utf-8")
    telemetry = (root / "extensions/youtube/src/main/java/app/spanishstudy/vot/OpenRouterTelemetry.java").read_text(encoding="utf-8")
    ...
    require(telemetry, "openRouterRecoverySplits=", "split telemetry")
    require(telemetry, "openRouterTransportRetries=", "transport retry telemetry")
    require(telemetry, "openRouterGoogleFallbackSucceeded=", "fallback telemetry")
    require(telemetry, "openRouterLastRecovery=", "last recovery telemetry")
