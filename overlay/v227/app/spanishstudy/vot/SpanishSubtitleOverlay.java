...
/**
 * Display-only bilingual subtitles over stock Morphe VOT.
 *
 * Morphe remains the sole authority for segmentation, translation, speech scheduling, playback
 * rate, seek behavior, Edge synthesis, cache and prefetch. This class only chooses which already
 * existing Morphe segment text to display. Long text is paginated instead of clipped by maxLines.
 */
final class SpanishSubtitleOverlay {
    ...
    static void update(Activity a, long timeMs) {
        ...
        boolean hasSpanish = translated != null
                && translated.lang != null
                && translated.lang.toLowerCase().startsWith("es")
                && !es.isBlank();
        boolean hasEnglish = source.lang != null
        ...
        // If Morphe is audibly speaking this segment, use Morphe's own adjusted TTS window rather
        // than the original caption visibility window. This is read-only and prevents subtitles
        // from disappearing while stock Morphe is still speaking the same text.
        ...
        cardView.setVisibility(translatedView.getVisibility() == View.VISIBLE
                || sourceView.getVisibility() == View.VISIBLE ? View.VISIBLE : View.GONE);

        if (pageIndex >= 0 && pageCount > 1
                && (index != lastSegment || pageIndex != lastPage || pageCount != lastPageCount)) {
            lastSegment = index;
            lastPage = pageIndex;
            lastPageCount = pageCount;
            SpanishStudyDiagnostics.record("SUBTITLE-PAGE",
                    "index=" + index + " page=" + (pageIndex + 1) + "/" + pageCount
    ...
    private static void ensureAttached(Activity a) {
        if (cardView != null && translatedView != null && sourceView != null
                && activity == a && cardView.getParent() != null) return;
    ...
    private static void updateLayout(Activity a) {
        ...
        ViewGroup.LayoutParams raw = cardView.getLayoutParams();
